...
export const useTask = () => {
  ...
  if (!context)
    throw new Error("useTask tiene que utilizarse con TaskContextProvider");
  return context;
};
  ...
  const updateTask = async (idTask, updateTask) => {
    ...
    try {
      ...
      if (folderID !== undefined && folderID !== "") {
        await supabase.from("folder_tasks").delete().eq("task_id", idTask);
        ...
        if (folderID && folderID !== "0") {
          ...
          if (relationError) {
            console.error(
              "Error al actualizar relación con carpeta:",
              relationError,
            );
