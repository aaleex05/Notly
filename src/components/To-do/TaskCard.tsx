...
function DeleteAlert({ onDelete }: DeleteAlertProps) {
  return (
    <Dialog>
      ...
      <DialogContent className="dark w-90 h-45">
        ...
        <div className="flex gap-3 justify-end items-center">
          <DialogClose>
            <span className="bg-primary border-1 border-border text-white/80 px-4 py-2 hover:text-white hover:bg-[#1c1c1c] rounded-md text-sm font-medium transition-all cursor-pointer disabled:opacity-50 outline-none">
              Cancelar
            </span>
          </DialogClose>
      ...
      <div className="h-full flex flex-col rounded-lg shadow-sm border bg-primary border-border">
        <div className="p-3 sm:p-5 space-y-6 sm:space-y-3 flex-grow">
          <div className="flex justify-between items-start gap-2 sm:gap-3">
            <h1 className="text-sm sm:text-lg font-medium break-words  flex-1 leading-tight sm:leading-normal">
              {task.name}
            </h1>
            <button
          ...
          <p className="text-xs sm:text-base break-words line-clamp-2 text-white/70 leading-snug sm:leading-relaxed">
            {task.description}
          </p>
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            <span
              className={`text-xs sm:text-sm py-0.5 sm:py-1 px-2.5 sm:px-3 rounded-full ${status.className}`}
            >
              {status.label}
            </span>
            <span
              className={`text-xs sm:text-sm py-0.5 sm:py-1 px-2.5 sm:px-3 rounded-full ${priority.className}`}
            >
              {priority.label}
            </span>
          </div>
        ...
        <div className="text-white/60 flex p-2.5 sm:p-4 border-t border-border">
          <span className="font-medium flex">
            {formatDateString(
              task.expirationDate ? new Date(task.expirationDate) : null,
            )}
          </span>
