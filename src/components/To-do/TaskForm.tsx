  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      ...
      <select
        ...
        className="p-2 rounded-lg border-1 border-border py-2 bg-primary focus:outline-2 focus:border-1 focus:border-[#797979] focus:outline-[#525252]"
      >
        <option value="default" disabled className="placeholder:text-amber-300">
      ...
      <Popover>
        <PopoverTrigger asChild>
          ...
          >
            ...
            {taskData.date
              ? format(new Date(taskData.date), "dd/MM/yyyy")
              : "Seleccionar fecha"}
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-auto p-0 bg-primary">
          <Calendar
            ...
            onSelect={(date) => {
              if (!date) return setTaskData({ ...taskData, date: null });
              ...
              setTaskData({ ...taskData, date: localDate.toISOString() });
            }}
        ...
        {taskData.date ? (
          <div>
            ...
            <span className="font-medium">
              {formatDateString(taskData.date ? new Date(taskData.date) : null)}
            </span>
            .
          </div>
        ) : (
